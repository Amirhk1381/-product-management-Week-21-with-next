...
function ProductsList() {
  ...
  useEffect(() => {
    ...
    const p = Number(queryPage) || 1;
    const s = typeof querySearch === "string" ? querySearch : "";
    if (p !== page) setPage(p);
    if (s !== search) setSearch(s);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [router.isReady, queryPage, querySearch]);
  const handlePageChange = (p) => {
    ...
    if (search && search.trim() !== "") {
      ...
      router.push({ pathname: "/products", query: { page: p.toString() } });
    }
  };
  useEffect(() => {
    setPage(1);
    ...
    if (debouncedSearch.trim() === "") {
      ...
      router.push({
        pathname: "/products",
        query: { page: "1", search: debouncedSearch },
      });
    }
  }, [debouncedSearch]);
